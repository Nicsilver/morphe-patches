 ...
 *
 * <p>The comment count is captured from the Shorts action bar buffer and is the active skip
 * signal. The like count (from Return YouTube Dislike) is currently dormant: upstream moved RYD
 * into the shared extension module, which cannot reference this youtube-only patch to report it.
 * {@link #onShortLikeCount} is kept as a ready injection point should that feed be re-wired.
 */
...
public final class SkipLowEngagementShortsPatch {
    ...
    /**
     * Injection point for the Shorts like count. Currently unwired: upstream relocated Return
     * YouTube Dislike to the shared module, which cannot call into this youtube-only patch. Kept
     * so the like-count skip signal can be restored via a cross-module callback if desired.
     */
